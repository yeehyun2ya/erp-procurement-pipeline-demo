...
현재 구현은 이슈 13, 동일 견적 JSON을 A/B/C 회사 config로 각각 실행해 서로 다른 경로를 비교하고, 외부 시스템 위임이 필요한 경로에서는 adapter 기반 mock 응답을 출력한 뒤 FastAPI 응답으로 HTML 목업의 4번째 SRM 단계를 다르게 제어하는 데모까지 포함합니다.
